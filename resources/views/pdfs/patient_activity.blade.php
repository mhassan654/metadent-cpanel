...
<head>
    ...
    <title>Patient Activity Logs</title>
    <style>
        .text-danger strong {
            color: #FF782E;
        }
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            @foreach ($activities as $activity)
                                <tr>
                                    <td class="col-md-3">{{ $activity->created_at->format('d-m-Y') }}</td>
                                    <td class="col-md-3">{{ $activity->created_at->format('h:i:s') }}</td>
                                    <td class="col-md-3">{{ $activity?->section }}</td>
                                    <td class="col-md-3">{{ $activity?->subject }}</td>
                                    <td class="col-md-3">{{ $activity?->action }}</td>
                                    <td class="col-md-3">
                ...
                <div class="row">
                    <div class="receipt-header receipt-header-mid receipt-footer">
                        <div class="col-xs-8 col-sm-8 col-md-8 text-left">
                            <div class="receipt-right">
                                <p><b>Patient Number :</b> {{ $patient[0]->unique_identifier }}</p>
                                <p><b>Patient Name :</b> {{ $patient[0]->first_name . ' ' . $patient[0]->last_name }}
                                </p>
                                <h5 style="color: rgb(140, 140, 140); margin-top: 20px;">Thank you very much!!!</h5>
                            </div>
                        </div>
