                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            @foreach ($patients as $patient)
                                @php
                                    $last_log_index = \App\Models\LogActivity::where('patient_id', $patient->id)
                                        ->with('user:id,first_name,last_name,maiden_name')
                                        ->latest()
                                        ->first();
                                @endphp
                                <tr>
                                    <td class="col-md-3">{{ $patient->first_name }} {{ $patient->middle_name }}
                                        {{ $patient->last_name }}</td>
                                    <td class="col-md-3">{{ $last_log_index?->created_at->format('d-m-Y') }}</td>
                                    <td class="col-md-3">{{ $last_log_index?->action }}</td>
                                    <td class="col-md-3">{{ $last_log_index?->subject }}</td>
                                    <td class="col-md-3">
                                        {{ $last_log_index?->user?->first_name . ' ' . $last_log_index?->user?->maiden_name . ' ' . $last_log_index?->user?->last_name }}
                                    </td>
                                    <td class="col-md-3">{{ $patient->BSN }}</td>
                                    <td class="col-md-3">{{ $patient->birth_date }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
