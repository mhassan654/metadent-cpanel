            ...
            <div class="receipt-main col-xs-10 col-sm-10 col-md-6 col-xs-offset-1 col-sm-offset-1 col-md-offset-3">
                ...
                <div style="margin-top: 5px">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            @foreach ($final_appointments as $appointment)
                                <tr>
                                    <td class="col-md-3">
                                        {{ !is_null($appointment->treatmentType) ? $appointment->treatmentType->title : $appointment?->appointmentType?->title }}
                                    </td>
                                    <td class="col-md-3">{{ $appointment?->status?->status }}</td>
                                    <td class="col-md-3">{{ $appointment?->date }}</td>
                                    <td class="col-md-3">{{ $appointment?->slots[0]['start-time'] }} -
                                        {{ $appointment->slots[0]['end-time'] }}</td>
                                    <td class="col-md-3">{{ $appointment?->source?->source }}</td>
                                    <td class="col-md-3">
                ...
                <div class="row">
                    <div class="receipt-header receipt-header-mid receipt-footer">
                        <div class="col-xs-8 col-sm-8 col-md-8 text-left">
                            <div class="receipt-right">
                                <p>
                                    <b>Patient Number :</b> {{ $patient[0]->unique_identifier }}
                                </p>
